...
([.github/workflows/cd.yml](../.github/workflows/cd.yml)), which builds the image, runs the
Trivy scan as a **hard gate**, and only then pushes to **GHCR** (`ghcr.io/davamix/calendar`).
No image is published on ordinary pushes to `main`.
...
git push origin v0.1.0               # ← triggers build → scan → publish
```
...
- `ghcr.io/davamix/calendar:X.Y.Z` and `:X.Y`
- `ghcr.io/davamix/calendar:sha-<commit>`
- `ghcr.io/davamix/calendar:latest` (re-pointed to this build)
