...
The app connects to Postgres as **`calendar_app`** — a `LOGIN NOSUPERUSER NOCREATEROLE
NOCREATEDB NOREPLICATION` role that **owns the `calendar` database and its objects** (so EF
`MigrateAsync` at startup can create/alter its own tables) but has no cluster-wide rights. The
bootstrap superuser (`POSTGRES_USER`, default `calendar`) is used only for administration /
...
- `30-app-role.sh` — creates `calendar_app` and hands it the `calendar` database + `public`
  schema, so the app migrates as `calendar_app`, owning everything it creates.

Set before first start (`.env`):

```
POSTGRES_PASSWORD=<admin password>          # bootstrap superuser
CALENDAR_APP_DB_PASSWORD=<strong password>  # the calendar_app role
ConnectionStrings__Calendar=Host=db;Port=5432;Database=calendar;Username=calendar_app;Password=<CALENDAR_APP_DB_PASSWORD>
```
...
```bash
docker exec -it calendar-db psql -U calendar -d postgres \
  -c "CREATE ROLE calendar_app LOGIN NOSUPERUSER NOCREATEROLE NOCREATEDB NOREPLICATION PASSWORD '<pwd>';"
# then transfer ownership of the calendar DB + its tables/sequences to calendar_app,
# and switch ConnectionStrings__Calendar to Username=calendar_app.
```

(See the Acopio reference runbook for the full ownership-transfer SQL if needed.) Reversible:
the bootstrap superuser still exists; revert the connection string to roll back.
...
against its own database server/instance, so this doesn't widen the app's blast radius. The app's
own `calendar_app` role remains least-privilege (no `CREATEROLE`).
